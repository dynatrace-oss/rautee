...
class HostnameRule(Rule):
    ...
    def __init__(self, value: str, operator: str, minimum_score: float, stop_after_match: bool, params: Dict[str, str]):
        super(HostnameRule, self).__init__(value, operator, minimum_score, stop_after_match, params)
    ...
    def _get_match_and_remainder(self, security_data: SecurityData, matching_hostnames: Set[str]):
        remainder = copy.deepcopy(security_data)
        security_data.remove_affected_entities_by_related_hostnames(matching_hostnames, False)
        remainder.remove_affected_entities_by_related_hostnames(matching_hostnames, True)
